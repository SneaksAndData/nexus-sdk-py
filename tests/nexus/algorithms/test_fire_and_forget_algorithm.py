"""Tests for FireAndForgetAlgorithm."""
...
class FireAndForgetTest(FireAndForgetAlgorithm):
    """FireAndForgetAlgorithm implementation for testing."""
...
@pytest.mark.asyncio
async def test__fire_and_forget_algorithm_run__unit_test(inputs: InputTest, expected: OutputTest):
    """
    Test FireAndForgetAlgorithm.run logic:
    ...
    * 3) Algorithm spawns a single remote algorithm correctly.
    ...
    # Act
    with patch(
        "nexus_client_sdk.nexus.algorithms.fire_and_forget.NEXUS_FRAMEWORK_CONFIGURATION",
        mock_config,
    ):

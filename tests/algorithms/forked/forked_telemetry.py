...
def tags_from_payload(
    payload: TestForkedAlgorithmPayload, _: NexusConfigurationModel, __: NexusDefaultArguments
) -> dict[str, str]:
    return {"x_tag": str(sum(payload.x))}


def enrich_from_payload(
    payload: TestForkedAlgorithmPayload, _: NexusConfigurationModel, run_args: NexusDefaultArguments

...
class TestUserAnalyticsTelemetry(UserTelemetryRecorder):
    ...
    def __init__(
        self,
        algorithm_payload: TestForkedAlgorithmPayload,
...
def tags_from_payload(payload: TestForkedAlgorithmPayload, _: NexusDefaultArguments) -> dict[str, str]:
    return {"x_tag": str(sum(payload.x))}


def enrich_from_payload(
    payload: TestForkedAlgorithmPayload, run_args: NexusDefaultArguments
...
def tag_metrics(payload: TestForkedAlgorithmPayload, _: NexusDefaultArguments) -> dict[str, str]:
    return {
